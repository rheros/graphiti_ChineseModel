...
# Running tests: pytest -xvs tests/llm_client/test_qwen_client_int.py
...
pytestmark = pytest.mark.skipif(
    'TEST_QWEN_API_KEY' not in os.environ,
    reason='Qwen API key not available',
)
...
async def test_generate_simple_response():
    """Test generating a simple response from the Qwen API."""
    if 'TEST_QWEN_API_KEY' not in os.environ:
        pytest.skip('Qwen API key not available')
    ...
    try:
        ...
        pytest.skip(f'Test skipped due to Qwen API error: {str(e)}')
...
if __name__ == '__main__':
    pytest.main(['-v', 'test_qwen_client_int.py'])