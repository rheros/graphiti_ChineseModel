...
# Running tests: pytest -xvs tests/llm_client/test_qwen_client.py
...
def qwen_client(mock_openai_client):
    """Fixture to create a QwenClient with a mocked AsyncOpenAI client."""
    config = LLMConfig(
    ...
    client = QwenClient(config=config, cache=False)
...
class TestQwenClientInitialization:
    ...
    def test_init_with_default_model(self):
        ...
        # QwenClient inherits DEFAULT_MODEL from BaseOpenAIClient
        assert client.model == 'gpt-4.1-mini'

    @patch.dict(os.environ, {'QWEN_API_KEY': 'env_api_key'})
    def test_init_without_config(self):
        """Test initialization without a config, using environment variable."""
        # Since QwenClient doesn't have special env var handling in __init__,
        # this test verifies it falls back to LLMConfig's behavior
        client = QwenClient(cache=False)

        # LLMConfig looks for OPENAI_API_KEY by default, not QWEN_API_KEY
...
@pytest.mark.asyncio
async def test_create_structured_completion_strips_reasoning_for_non_reasoning_models(qwen_client, mock_openai_client):
    """Test that reasoning/verbosity parameters are stripped for non-reasoning models."""
    await qwen_client._create_structured_completion(
        model='qwen-turbo',
        messages=[],
        temperature=0.4,
        max_tokens=64,
    ...
    # Qwen models don't support reasoning/verbosity parameters
    assert 'reasoning' not in call_args
    assert 'text' not in call_args


@pytest.mark.asyncio
async def test_create_completion_with_json_format(qwen_client, mock_openai_client):
    """Test regular completion with JSON response format."""
    await qwen_client._create_completion(
        model='qwen-turbo',
        messages=[],
        temperature=0.7,
        max_tokens=128,
...
@pytest.mark.asyncio
async def test_generate_response_with_response_model(qwen_client, mock_openai_client):
    ...
    result = await qwen_client.generate_response(
        messages=messages,
        response_model=DummyResponseModel
    )
...
@pytest.mark.asyncio
async def test_generate_response_without_response_model(qwen_client, mock_openai_client):
    ...
    messages = [Message(role='user', content='Test message')]
    result = await qwen_client.generate_response(messages=messages)
...
if __name__ == '__main__':
    pytest.main(['-v', 'test_qwen_client.py'])