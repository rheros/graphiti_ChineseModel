...
# connecting to Neo4j database and DeepSeek API
#################################################
...
# DeepSeek API connection parameters
...
async def main():
    ...
    # INITIALIZATION
    #################################################
    # Connect to Neo4j and DeepSeek API, then set up
    ...
    # Create OpenAI Embedder client (can use with DeepSeek since APIs are compatible)
    embedder_client = OpenAIEmbedder()

    # Initialize Graphiti with Neo4j connection and DeepSeek client
    graphiti = Graphiti(
        neo4j_uri,
        neo4j_user,
        neo4j_password,
